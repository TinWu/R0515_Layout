...
namespace DotWeb
{
    ...
    public abstract class BaseController : SourceController
    {
        ...
        //訂義檔案上傳路行樣板
        protected string sysUpFilePathTpl = "~/_Upload/{0}/{1}/{2}/{3}/{4}";
        protected string sysUpFilePathTplS = "~/_Upload/{0}/{1}/{2}/{3}";
        //訂義檔案刪除路徑樣板
        protected string sysDelSysId = "~/_Upload/{0}/{1}/{2}";
        ...
        protected void HandFileSave(string filename, int Id, FilesUpScope fp, string filekind, string name1, string name2)
        {
            ...
            string tpl_Org_FolderPath = string.Format(sysUpFilePathTpl, name1, name2, Id, filekind, "OriginFile");
            ...
            FileStream write_stream = new FileStream(Org_Path + "\\" + filename, FileMode.Create);
            BinaryWriter bin_write = new BinaryWriter(write_stream);
            bin_write.Write(fileContents);
        ...
        protected void HandImageSave(string filename, int Id, ImageUpScope fp, string filekind)
        {
            HandImageSave(filename, Id, fp, filekind, getArea, getController);
        }
        protected void HandImageSave(string filename, int id, ImageUpScope fp, string filekind, string name1, string name2)
        {
            ...
            //後台管理的代表小圖
            string tpl_Rep_FolderPath = String.Format(sysUpFilePathTpl, name1, name2, id, filekind, "RepresentICON");
            ...
            if (fp.Parm.Count() > 0)
            {
                ...
                {
                    tpl_Rep_FolderPath = String.Format(sysUpFilePathTplS, name1, name2, id, filekind);
        ...
        protected FilesObject[] ListSysFiles(int Id, string filekind)
        {
            return ListSysFiles(Id, filekind, true);
        }
        protected FilesObject[] ListSysFiles(int Id, string filekind, Boolean IsImageList)
        {
            String tpl_FolderPath = String.Empty;
            String Path = String.Empty;

            String AccessFilesKind = filekind == "" ? "DocFiles" : filekind;
            ...
            if (Directory.Exists(Path))
            {
                foreach (String fileString in Directory.GetFiles(Path))
                {
                    ...
                    FilesObject fo = new FilesObject() { FileName = fi.Name, FilesKind = filekind, RepresentFilePath = Url.Content(tpl_FolderPath + "/" + fi.Name) };
        ...
        protected FilesObject[] ListSysFiles(int Id, string filekind, bool IsImageList, string name1, string name2)
        {
            string tpl_FolderPath = String.Empty;
            string Path = String.Empty;

            string AccessFilesKind = filekind == "" ? "DocFiles" : filekind;
            tpl_FolderPath = string.Format(sysUpFilePathTpl, name1, name2, Id, AccessFilesKind, "OriginFile");
            Path = Server.MapPath(tpl_FolderPath);
            IsImageList = false;
            ...
            if (Directory.Exists(Path))
            {
                ...
                if (CheckFiles.Count() > 0)
                {
                    ...
                    GetFirstFileToCheck.Extension.ToLower().Contains("bmp"))
                    {
                        tpl_FolderPath = string.Format(sysUpFilePathTpl, name1, name2, Id, AccessFilesKind, "RepresentICON");
                        Path = Server.MapPath(tpl_FolderPath);
                        IsImageList = true;
                    }
                }
            ...
            if (Directory.Exists(Path))
            {
                foreach (String fileString in Directory.GetFiles(Path))
                {
                    ...
                    if (IsImageList)
                    {
                        String org_tpl_FolderPath = String.Format(sysUpFilePathTpl, name1, name2, Id, AccessFilesKind, "OriginFile");
                        Path = Server.MapPath(org_tpl_FolderPath);
                        fi = new FileInfo(Path + "/" + fi.Name);
                        fo.OriginFilePath = Url.Content(org_tpl_FolderPath + "/" + fi.Name);
                        fo.Size = fi.Length;
        ...
        protected void DeleteSysFile(int Id, string fileskind, string filename, ImageUpScope im, string name1, string name2)
        {
            string SystemDelSysIdKind = "~/_Upload/{0}/{1}/{2}/{3}";
            string tpl_FolderPath = Server.MapPath(String.Format(SystemDelSysIdKind, name1, name2, Id, fileskind));

            string masterfile = tpl_FolderPath + "/" + filename;
